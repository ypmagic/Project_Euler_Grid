...
public class GridProduct {
    ...
    public static void main (String[] args) {
        ...
        ProductRight one = new ProductRight (grid);
        ProductDown two = new ProductDown (grid);
        ProductDiagonalRight three = new ProductDiagonalRight (grid);
        ProductDiagonalLeft four = new ProductDiagonalLeft (grid);

        int highestOne = one.calculateHighestProduct();
        int highestTwo = two.calculateHighestProduct();
        int highestThree = three.calculateHighestProduct();
        int highestFour = four.calculateHighestProduct();
        int highestProduct = 0;

        if (highestOne > highestTwo && highestOne > highestThree && highestOne > highestFour) {
            highestProduct = highestOne;
        }
        else if (highestTwo > highestOne && highestTwo > highestThree && highestTwo > highestFour) {
            highestProduct = highestTwo;
        }
        else if (highestThree > highestOne && highestThree > highestTwo && highestThree > highestFour) {
            highestProduct = highestThree;
        }
        else if (highestFour > highestOne && highestFour > highestTwo && highestFour > highestThree) {
            highestProduct = highestFour;
        }

        System.out.println("The highest product of four adjacent numbers in the 20x20 grid is: " + highestProduct);
    }
}
